...
lowIE | boolean | | | 인터넷익스플로러6, 7, 8에서 반응형 사용여부 입니다.
pauseOnArrowClick | boolean | | | 이전 또는 다음버튼 클릭 후 일시정지 여부 입니다.
play | string | | | 재생에 대한 문자를 지정합니다.
pause | string | | | 일시정지에 대한 문자를 지정합니다.
current | element \|\| jQueryElement | | | 현재 슬라이드 위치를 표기할 요소 입니다.
total | element \|\| jQueryElement | | | 슬라이드의 갯수를 표기할 요소 입니다.
customState | function | state | object[current : string \|\| number, total : string \|\| number] | 현재 슬라이드 위치와 슬라이드 갯수를 조정합니다.

- 슬릭이 셋팅되어 있는 상태에서 재호출 시 오류제거
- 매개변수에 이상한 값이 올 경우 오류제거
- 코어 값에 이상한 값이 올 경우 오류제거
- 코어 값에 요소가 여러개 들어왔을때 셋팅
